...
- **3D 트렁크 뷰**: 빈 곳을 드래그해 회전, 휠/핀치로 줌, 우클릭 드래그로 패닝. 가림 순서가 정확한 painter's algorithm 렌더링. 쏘렌토답게 그린 고정물(휠하우스 아치, 개구부 프레임·범퍼 턱·테일램프, 2열 등받이·헤드레스트, 카고 매트)과 짐 모양(원통 가방, 연질 가방, 쿨러, 수납함, 하드케이스)
...
npx playwright test          # 브라우저 E2E 34개, 약 12분 (build/web 필요, 스크린샷은 e2e/screenshots/)
...
|------|----|------|------|------|
| 쏘렌토 5인승 | 109~138 (138 은 뒤쪽 38cm) | 107 (+2열 슬라이드 0~27) | 79~82 | 줄자 실측 8건 + 기아 도면 + 실제 적재 사례(ADAC 980 L, 골프백·캐리어 후기)로 보정 (`backlog/sorento-mq4-measurements.md`) |
| 쏘렌토 7인승·3열 접음 | 위와 같음 | 위와 같음 | 위와 같음 | 5인승과 같은 상자, 바닥 아래 수납함만 다름 |
...
- `test/validation/` 실제 쏘렌토 적재 사례(증인 배치), 카탈로그 198개 단품 판정, 번들 판정 매트릭스, 충돌 일관성, 성능 가드
- `test/models/packer_invariants_test.dart` 8개 트렁크 × 무작위·실장비 세트 × 3전략 불변식 (충돌·부양·닫힘·무게·연질·결정성)
...
- `e2e/*.spec.ts` 실제 웹 빌드 브라우저 테스트 34개: 번들별 판정 색과 시간, 미적재→2열 제안 적용, 테일게이트 걸림↔실행 취소, 편집·단축키·카메라, 자동배치 대안·스텝 뷰, 차종·2열 메뉴, 새로고침 복원·이름 저장·손상 저장본, 폰 10개(전체 화면 장비 페이지, 시트를 따라가는 캔버스, 선택 도구 띠, 결과 시트, 가로·소형·태블릿), 리사이즈·연타·20회 내구
- `test/widgets/simulator_phone_test.dart` 폰 UX 사양(`backlog/phone-ux-w9.md`) 독립 검증 75개, `gear_picker_test.dart` 전체 화면 장비 페이지 16개, `test/render3d/label_policy_test.dart`·`caption_style_test.dart` 폰 압축 라벨·캡션
